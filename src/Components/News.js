...
export default class News extends Component {
  ...
  constructor(){
    ...
    let url = "https://newsapi.org/v2/top-headlines?country=in&category=business&apiKey=f9cb9a1868234c8780e38d634b2f7325&pageSize=20"
  ...
  nextClick = async()=>{
    // console.log(Math.ceil(this.state.totalResults/20))t=70 3.5 pages round and bcm 4pgs
    if(this.state.page + 1 > Math.ceil(this.state.totalResults/20))
    {
        //not show next page
    }
    else{
    let url = `https://newsapi.org/v2/top-headlines?country=in&category=business&apiKey=f9cb9a1868234c8780e38d634b2f7325&page= ${this.state.page + 1}&pageSize=20`
    this.setState({loading:true})
    let data = await fetch(url)
    let parseddata = await data.json()
    // console.log(parseddata)
    ...
    return (
      <div className="container my-3">
        ...
        <div className='container' >
            <button type='button' className='btn btn-dark mx-3  float-end' onClick={this.nextClick}> Next &rarr;</button>
            <button type='button' disabled={this.state.page<=1} className='btn btn-dark mx-3  float-end' onClick={this.prevClick}> &larr; Previous</button>
        </div>
        {/* 
